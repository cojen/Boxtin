...
/**
 * Immutable map which uses the descriptor of a MemberRef as the lookup key, and a byte array
 * as the stored key.
 *
...
class MemberRefDescriptorMap<V> extends MemberRefLookupMap<V> {
    ...
    protected int lookupHash(MemberRef lookupKey) {
        return Utils.hash(lookupKey.buffer(),
                          lookupKey.descriptorOffset(), lookupKey.descriptorLength());
    }

    @Override
    protected boolean matches(MemberRef lookupKey, byte[] existingKey) {
        int offset = lookupKey.descriptorOffset();
